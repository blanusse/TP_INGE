...
- Name: registro-completo.spec.ts >> Registro — Dador personal (happy path) >> DADO formulario dador personal CUANDO completa datos válidos ENTONCES redirige a verify-email
- Location: tests/e2e/registro-completo.spec.ts:56:7
...
Call log:
  - waiting for getByText('Persona física') to be visible
...
```ts
  1   | /**
  2   |  * E2E — Registro completo de usuarios nuevos
  3   |  *
  4   |  * Flujo: /login → "Registrarse gratis" → selección de perfil → formulario → validaciones → submit
  5   |  * No se puede verificar email (no hay inbox), el test termina en /verify-email.
  6   |  */
