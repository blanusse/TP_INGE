...
- Name: dador.spec.ts >> Dador — Mis cargas >> muestra las tabs de filtro de cargas
- Location: tests/e2e/dador.spec.ts:55:7
...
Locator: getByRole('button', { name: 'Todas' }).or(getByText('Todas')).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: 'Todas' }).or(getByText('Todas')).first()
