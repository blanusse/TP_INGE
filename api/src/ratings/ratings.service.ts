  ...
  async submitRating(
    ...
    if (!load || load.status !== 'delivered') {
      throw new BadRequestException(
        'Solo se puede calificar después de que el viaje fue entregado.',
      );
    }
    ...
    let toUserId: string;
    if (isDriver) {
      const shipperRecord = await this.shippersRepo.findOne({
        where: { id: load.shipper_id },
      });
