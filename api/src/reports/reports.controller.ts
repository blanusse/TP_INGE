import {
  ...
  Headers, ForbiddenException, UseInterceptors, UploadedFile,
} from '@nestjs/common';
...
import { ReportReason, ReportStatus, AdminAction } from '../entities/report.entity';
...
export class ReportsController {
  ...
  createReport(
    @Request() req,
    @Body() body: { reported_id: string; reason: ReportReason; description: string; evidence_url?: string },
  ) {
  ...
  updateReport(
    ...
    @Param('id') id: string,
    @Body() body: { status?: ReportStatus; admin_action?: AdminAction; admin_notes?: string },
  ) {
    if (secret !== process.env.INTERNAL_SECRET) throw new ForbiddenException();
